...
class CanReceiver(Node):
    ...
    def format_inverter_status(self, status):
        ...
        INVstatus = status1 & 0x0F
        HVstatus = (status1 >> 4) & 0x0F
        ERRORstatus = status2
        statusText1 = ''
        statusText2 = ''
        statusText3 = ''
        ...
        return f"INV: {statusText1}({status1Hex})   -   HV: {statusText2}({status2Hex})  -  ERROR: {statusText3}({ERRORstatus})"
    ...
    def update_dashboard(self):
        ...
        print(f"[VCU]          Steer: {self.format_value(vcu['steer'], '{:.2f}°'):>8} "
              ...
              f"Brake: {self.format_value(vcu['brake'], '{:.2f}'):>8} "
